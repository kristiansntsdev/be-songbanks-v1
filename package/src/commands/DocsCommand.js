...
Features:
  ...
  • Laravel-style route patterns
    ...
    generateDocs() {
        ...
        console.log(`📝 Found ${controllerNames.length} controller(s): ${controllerNames.join(', ')}`);
        ...
        console.log(`✅ Generated complete API documentation: swagger/swagger.json`);
    ...
    inferHttpMethod(methodName) {
        ...
        if (lowerMethod.includes('index') || lowerMethod.includes('show') || lowerMethod.includes('get')) {
            return 'get';
        }
        if (lowerMethod.includes('store') || lowerMethod.includes('create') || lowerMethod.includes('post')) {
            return 'post';
        }
    ...
    addControllerPaths(swaggerSpec, controllerName, controller) {
        ...
        Object.keys(controller.methods).forEach(methodName => {
            ...
            const route = this.generateRoute(controllerName, methodName, method.httpMethod);
            
            if (!swaggerSpec.paths[route.swaggerPath]) {
                swaggerSpec.paths[route.swaggerPath] = {};
            }
            
            swaggerSpec.paths[route.swaggerPath][method.httpMethod] = this.generateOperation(
                controllerName, 
                methodName, 
                method.httpMethod,
                route
    ...
    generateRoute(controllerName, methodName, httpMethod) {
        ...
        if (lowerMethod.includes('index') || (httpMethod === 'get' && !lowerMethod.includes('show'))) {
            return {
                path: `/api/${entity}s`,
    ...
    generateOperation(controllerName, methodName, httpMethod, route) {
        ...
        if (route.swaggerPath.includes('{')) {
            const paramName = route.swaggerPath.includes('{user_id}') ? 'user_id' : 'id';
            operation.parameters = [{
                in: "path",
                name: paramName,
                required: true,
                schema: { type: "string" },
                description: paramName === 'user_id' ? 'The user ID' : `The ${entity} ID`
            }];
        }
    ...
    generateDescription(methodName, httpMethod, entity) {
        ...
        if (lowerMethod.includes('login')) return 'Authenticate user with email and password';
        if (lowerMethod.includes('logout')) return 'Logout authenticated user';
        if (lowerMethod.includes('index')) return `Retrieve paginated list of ${entity}s`;
        if (lowerMethod.includes('show') || lowerMethod.includes('byid')) {
            return `Retrieve a specific ${entity} by its ID`;
        }
        if (httpMethod === 'post') return `Create a new ${entity} with provided data`;
    ...
    getStandardResponses(controllerName, methodName) {
        ...
        if (controllerName === 'AuthController') {
            ...
            if (methodName.includes('logout')) {
                responses["200"].content["application/json"].schema = { "$ref": "#/components/schemas/LogoutResponse" };
                responses["401"] = {
        ...
        return {
            ...
            "AccountAccessDeniedError": {
                ...
                "properties": {
                    ...
                    "error": { "type": "string", "example": "Your account status is inactive. Please contact administrator." }
                }
            },
            "User": {
                "type": "object",
                "properties": {
                    ...
                    "status": { "type": "string", "enum": ["active", "pending", "suspend", "request", "guest"], "example": "active" }
                }
            },
            "LoginRequest": {
                "type": "object",
            ...
            "CreateTagRequest": {
                ...
                "required": ["name"],
                "properties": {
                    "name": { "type": "string", "example": "Rock", "description": "The name of the tag" },
                    "description": { "type": "string", "example": "Rock music genre", "description": "Optional description of the tag" }
                }
            },
            "UpdateTagRequest": {
                "type": "object",
