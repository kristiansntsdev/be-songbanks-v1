...
Available controllers:`);
  ...
  generateByController(controllerName) {
    ...
    console.log(
      `🔍 Generating Swagger docs for ${controllerName} in routes/api.js...`
    );
  ...
  addControllerToRoutes(controllerName, controllerRoutes) {
    ...
    const controllerImportPattern = new RegExp(
    ...
    if (!controllerImportPattern.test(routesContent)) {
      ...
      const lastImportMatch = routesContent.match(
        /const \w+Controller = require\('\.\.\/app\/controllers\/\w+Controller'\);/g
      );
      if (lastImportMatch) {
        const lastImport = lastImportMatch[lastImportMatch.length - 1];
        const lastImportIndex =
          routesContent.indexOf(lastImport) + lastImport.length;
        updatedContent =
    ...
    const existingPattern = new RegExp(
      `router\\.[a-z]+\\('[^']*',\\s*${controllerName}\\.[a-zA-Z]+\\);`,
      "g"
    );
    ...
    if (existingMatches.length > 0) {
      console.log(
        `⚠️  Found ${existingMatches.length} existing ${controllerName} routes. Removing duplicates...`
      );
      ...
      existingMatches.forEach((routeMatch) => {
        ...
        if (routeIndex !== -1) {
          ...
          if (lastSwaggerBlockStart !== -1) {
            const swaggerBlockEnd =
              updatedContent.indexOf("*/", lastSwaggerBlockStart) + 2;
            const routeEnd = routeIndex + routeMatch.length;

            // Remove swagger block and route line
            updatedContent =
              updatedContent.slice(0, lastSwaggerBlockStart) +
    ...
    if (controllerName === "TagController") {
      ...
      if (updatedContent.includes("# Tag Schemas")) {
        ...
        const securitySchemesIndex =
          updatedContent.indexOf("  securitySchemes:");
        if (tagSchemaStart !== -1 && securitySchemesIndex !== -1) {
          updatedContent =
            updatedContent.slice(0, tagSchemaStart) +
      ...
      if (securitySchemesIndex !== -1) {
        updatedContent =
          updatedContent.slice(0, securitySchemesIndex) +
          tagSchemas +
          "\n     " +
          updatedContent.slice(securitySchemesIndex);
      }
    ...
    const moduleExportsIndex = updatedContent.indexOf(
      "module.exports = router;"
    );
    if (moduleExportsIndex !== -1) {
      updatedContent =
        updatedContent.slice(0, moduleExportsIndex) +
        routeDocumentation +
    ...
    console.log(
      `✅ Added ${controllerRoutes.length} ${controllerName} endpoints to routes/api.js`
    );
  }
  ...
  generateMethodSummaryAndDescription(route) {
    ...
    const summaryMap = {
      ...
      getUserAccess: {
        ...
        description: "Retrieve user access list for admin management",
      },
      updateUserAccess: {
        summary: "Update user access",
        description: "Update user access status (active/suspend)",
      },
      apiLogin: {
        summary: "User login",
    ...
    const fallbackMap = {
      GET: {
        ...
        description: `Retrieve data from ${route.routerPath}`,
      },
      POST: {
        summary: `${httpMethod} ${route.routerPath}`,
        description: `Create new resource at ${route.routerPath}`,
      },
      PUT: {
        summary: `${httpMethod} ${route.routerPath}`,
        description: `Update resource at ${route.routerPath}`,
      },
      DELETE: {
        summary: `${httpMethod} ${route.routerPath}`,
        description: `Delete resource at ${route.routerPath}`,
      },
    };

    return (
      fallbackMap[httpMethod] || {
        summary: `${httpMethod} ${route.routerPath}`,
  ...
  updateMainRoutes(controllerRoutes) {
    ...
    Object.keys(existingRoutes).forEach((controllerName) => {
      if (controllerName !== controllerRoutes[0].controller) {
        Object.keys(existingRoutes[controllerName].methods).forEach(
          (methodName) => {
