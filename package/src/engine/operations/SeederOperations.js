...
class SeederOperations {
  ...
  async safeInsert(tableName, data, options = {}) {
    ...
    if (uniqueFields.length > 0) {
      console.log(
        `🔍 Automatic duplicate detection enabled for ${tableName} (checking: ${uniqueFields.join(", ")})`
      );
    }
    ...
    console.log(
      `📊 Processing complete for ${tableName} (${totalProcessed} records):`
    );
    ...
    if (results.skipped.length > 0) {
      console.log(
        `   ⚠️  Skipped: ${results.skipped.length} duplicates (${onDuplicate} strategy)`
      );
    }
  ...
  async checkExisting(tableName, record, uniqueFields) {
    ...
    try {
      ...
      const results = await this.queryInterface.sequelize.query(
        `SELECT * FROM ${tableName} WHERE ${Object.keys(whereCondition)
          .map((field) => `${field} = ?`)
          .join(" AND ")} LIMIT 1`,
        {
      ...
      console.warn(
        `Warning: Could not check existing records in ${tableName}: ${error.message}`
      );
      return null;
    }
  }
  ...
  async handleDuplicate(tableName, record, existing, strategy, results) {
    ...
    const identifyingField =
      Object.keys(record).find(
        (key) =>
          record[key] === existing[key] &&
          ["email", "name", "slug", "id"].includes(key)
      ) || Object.keys(record)[0];

    switch (strategy) {
      case "skip":
        console.log(
          `   ⏭️  Skipping duplicate: ${identifyingField}='${record[identifyingField]}'`
        );
      ...
      case "update":
        console.log(
          `   🔄 Updating duplicate: ${identifyingField}='${record[identifyingField]}'`
        );
      ...
      default:
        console.log(
          `   ❓ Unknown strategy for duplicate: ${identifyingField}='${record[identifyingField]}', skipping`
        );
        results.skipped.push({
          record,
          existing,
  ...
  async updateRecord(tableName, record, existing) {
    ...
    await this.queryInterface.bulkUpdate(tableName, updateData, {
      id: existing.id,
    });
  }
    ...
    try {
      if (checkForeignKeys) {
        ...
        if (foreignKeys.length > 0) {
          console.warn(
            `⚠️  Table '${tableName}' has foreign key references: ${foreignKeys.join(", ")}`
          );
          if (!options.force) {
            throw new Error(
              `Cannot truncate table '${tableName}' due to foreign key constraints`
            );
          }
        }
      }
  ...
  async getForeignKeyReferences(tableName) {
    try {
      ...
      console.warn(
        `Warning: Could not check foreign key references for ${tableName}`
      );
      return [];
    }
  }
