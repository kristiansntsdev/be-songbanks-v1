...
class Seeder {
        ...
        factory.saveModelInstance = async (instance) => {
            ...
            if (this.config.autoHandleDuplicates) {
                ...
                if (results.inserted.length > 0) {
                    ...
                    console.log(`⚠️  Factory instance skipped (duplicate): ${JSON.stringify(results.skipped[0].record)}`);
    ...
    async insert(tableName, data, options = {}) {
        ...
        if (this.config.autoHandleDuplicates) {
            const results = await this.operations.safeInsert(tableName, data, finalOptions);
            ...
            console.log(`📦 Inserted ${processedRecords.length} records into ${tableName}`);
            
            return { inserted: processedRecords, skipped: [], updated: [], errors: [] };
        }
    }
     ...
     */
    async insertIfNotExists(tableName, data, uniqueFields = this.config.defaultUniqueFields) {
        return await this.operations.insertIfNotExists(tableName, data, uniqueFields);
    }

    /**
     * Upsert data (insert or update existing)
     */
    async upsert(tableName, data, uniqueFields = this.config.defaultUniqueFields) {
    ...
    async create(overrides = {}) {
        try {
            ...
            for (const relationship of this.relationships) {
                factoryInstance = factoryInstance.has(relationship.name, relationship.count);
            }
