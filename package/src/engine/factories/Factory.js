...
class Factory {
        ...
        for (const [key, value] of Object.entries(definition)) {
            ...
            if (value instanceof Factory) {
                if (!this.expandRelationships) {
                    ...
                    expandedValue = recycled ? recycled.id : value.recycle(this.recycle).create().id;
                }
            }
            ...
            // Handle functions
                ...
                if (expandedValue instanceof Factory) {
                    expandedValue = this.expandRelationships 
                        ...
                        : null;
                } else if (expandedValue && typeof expandedValue === 'object' && expandedValue.id) {
    ...
    has(factory, relationship = null) {
        const relationshipName = relationship || this.guessRelationship(factory.modelName());
    ...
    hasAttached(factory, pivot = {}, relationship = null) {
        const relationshipName = relationship || this.guessRelationship(factory.modelName(), true);
    ...
    getModelClass() {
        ...
        try {
            ...
            throw new Error(`Could not resolve model class for factory ${factoryName}. Please set the 'model' property.`);
        }
    }
    ...
    async createChildren(model) {
        for (const relationship of this.has) {
            const { factory, relationship: relationshipName, pivot, type } = relationship;
            ...
            if (type === 'belongsToMany') {
                ...
                const children = await factory.recycle(this.recycle).for(model).createMany();
            }
        }
    }
    ...
    static resolveFactoryName(modelName) {
        ...
        try {
            ...
            throw new Error(`Could not find factory ${factoryName} for model ${modelName}`);
        }
    }
}
