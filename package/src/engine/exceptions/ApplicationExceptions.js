const { UnauthorizedException, ForbiddenException, NotFoundException, ConflictException } = require('./HttpExceptions');
...
class ModelNotFoundException extends NotFoundException {
    constructor(model, id = null) {
        const message = id ? `${model} with ID ${id} not found` : `${model} not found`;
        super(message);
        this.model = model;
        this.id = id;
    }
