...
class ResourceGenerator extends BaseGenerator {
  ...
  generate(resourceName, options = {}) {
    ...
    try {
      ...
      results.model = this.modelGenerator.generate(modelName, {
        resource: true,
      });
  ...
  showNextSteps(results) {
    ...
    console.log(`\n💡 Next steps:`);
    console.log(
      `   1. Create migration: sequelize-cli migration:generate --name create-${routes.plural}`
    );
    ...
    console.log(
      `   5. Generate Swagger docs: npm run swagger:controller generate ${resourceName}Controller`
    );
  }
}
