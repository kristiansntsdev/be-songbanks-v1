...
class TagService {
    static async getAllTags(options = {}) {
        ...
        const { count, rows } = await Tag.query()
            ...
            .when(withSongCount, q => q
                ...
                .applyOptions({
                    attributes: ['id', 'name', 'description', 'createdAt', 'updatedAt', [fn('COUNT', col('songs.id')), 'songCount']],
                    group: ['Tag.id'],
                    subQuery: false
                })
            )
    ...
    static paginatedResponse(rows, count, page, limit, key = 'tags') {
        return {
            [key]: rows,
