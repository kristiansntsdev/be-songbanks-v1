...
class TagFactory extends Factory {
    ...
    definition() {
        return {
            name: FactoryTypes.randomElement([
                'Rock', 'Pop', 'Folk', 'Blues', 'Country', 'Jazz', 'Gospel',
                'Acoustic', 'Electric', 'Alternative', 'Indie', 'Classic'
            ]),
    ...
    genre() {
        return this.state({
            name: FactoryTypes.randomElement(['Rock', 'Pop', 'Folk', 'Blues', 'Country', 'Jazz', 'Gospel']),
            description: FactoryTypes.computed((attributes) => `${attributes.name} music category`)
        });
    }
     ...
     */
    mood() {
        const moodNames = ['Happy', 'Sad', 'Energetic', 'Relaxing', 'Romantic', 'Spiritual'];
    ...
    instrument() {
        ...
        return this.state({
            ...
            description: FactoryTypes.computed((attributes) =>
                instrumentDescriptions[attributes.name] || `${attributes.name} featured music`
            )
        });
    }
    ...
    tempo() {
        return this.state({
            ...
            description: FactoryTypes.computed((attributes) => `${attributes.name} tempo songs`)
        });
    }
    ...
    era() {
        return this.state({
            name: FactoryTypes.randomElement(['Classic', 'Modern', 'Vintage', 'Contemporary']),
            description: FactoryTypes.computed((attributes) => `${attributes.name} era music`)
        });
    }
    ...
    popular() {
        return this.state({
            name: FactoryTypes.randomElement(['Top 40', 'Popular', 'Hit', 'Chart-topper']),
    ...
    rockWithSongs(songCount = 3) {
        return this.state({
            ...
            const songs = await SongFactory.new()
                .rock()
                .count(songCount)
                .create();
